...
Processes cargo-geiger JSON output to create badge files for shields.io. It analyzes
workspace crates only (excluding dependencies) and generates separate badge files for
overall unsafe percentage and individual categories.

cargo-geiger can be run manually to get results against custom targets, features, etc.
...
def generate_badges(metrics: Dict[str, Dict[str, int]], output_dir: Path) -> None:
    ...
    # Calculate totals for overall badge
